...
class AutoridadesService {
    ...
    // Listar todas las autoridades activas
    async listarAutoridades() {
        try {
            const autoridades = await autoridadesRepository.obtenerTodasLasAutoridades();
            return {
                success: true,
                data: autoridades,
    ...
    async obtenerAutoridadPorId(idAutoridad) {
        try {
            ...
            const autoridad = await autoridadesRepository.obtenerAutoridadPorId(idAutoridad);
    ...
    async crearAutoridad(datosAutoridad) {
        try {
            ...
            if (!orden_firma) {
                orden_firma = await autoridadesRepository.obtenerSiguienteOrden();
            } else {
                // Verificar si el orden ya existe
                const ordenExiste = await autoridadesRepository.verificarOrdenExistente(orden_firma);
                if (ordenExiste) {
            ...
            const idAutoridad = await autoridadesRepository.crearAutoridad(datosCompletos);
    ...
    async actualizarAutoridad(idAutoridad, datosActualizados) {
        try {
            ...
            const autoridadExistente = await autoridadesRepository.obtenerAutoridadPorId(idAutoridad);
            ...
            const actualizado = await autoridadesRepository.actualizarAutoridad(idAutoridad, datosCompletos);
    ...
    async eliminarAutoridad(idAutoridad) {
        try {
            ...
            const eliminado = await autoridadesRepository.eliminarAutoridad(idAutoridad);
    ...
    async actualizarOrdenesMultiples(actualizaciones) {
        try {
            ...
            const actualizado = await autoridadesRepository.actualizarOrdenesMultiples(actualizaciones);
