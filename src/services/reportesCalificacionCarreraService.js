...
class ReportesCalificacionCarreraService {
    ...
    async generarPDFReportePorCarrera(idPeriodo, idCarrera) {
        try {
            ...
            const pdfBuffer = await this.crearPDF(datosParaPDF, periodo, autoridades, true);
            return pdfBuffer;
        } catch (error) {
    ...
    async generarPDFReporteTodasCarreras(idPeriodo) {
        try {
            ...
            // Solo la firma de la vicerrectora
            const pdfBuffer = await this.crearPDF(datosParaPDF, periodo, autoridades, true);
            return pdfBuffer;
        } catch (error) {
    ...
    agregarSeccionFirmas(doc, autoridades, soloVicerrectora = false) {
        ...
        if (Array.isArray(autoridadesFiltradas) && autoridadesFiltradas.length > 0) {
            firmaData = {
                nombre: autoridadesFiltradas[0].nombre_autoridad,
                cargo: autoridadesFiltradas[0].cargo_autoridad
            };
        } else {
            firmaData = {
                nombre: 'Mg. Sandra Rivilla Requelme',
                cargo: 'VICERRECTORA'
            };
        }

        const firmaX = marginX;
        const centerX = firmaX + (availableWidth / 2);

        // Línea para la firma centrada
        doc.moveTo(centerX - 100, startY + 60)
            .lineTo(centerX + 100, startY + 60)
            .stroke();

        // Nombre centrado
        doc.fontSize(10).font('Helvetica-Bold');
        doc.text(firmaData.nombre, marginX, startY + 70, {
            width: availableWidth,
            align: 'center'
        });

        // Quitar el espacio extra entre nombre y cargo
        // doc.moveDown(1.2);

        // Cargo centrado
        doc.fontSize(9).font('Helvetica');
        doc.text(firmaData.cargo, marginX, startY + 85, {
            width: availableWidth,
            align: 'center'
        });
    }
