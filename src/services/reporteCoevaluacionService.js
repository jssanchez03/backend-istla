...
class ReporteCoevaluacionService {
    // 🔄 MODIFICADO: Obtener datos filtrados por carrera del coordinador
    ...
    async generarReportePDF(idPeriodo, cedulaCoordinador) {
        try {
            // Obtener carrera del coordinador
            const coordinadorData = await reporteCoevaluacionRepository.obtenerCarreraCoordinador(cedulaCoordinador);
            const idCarrera = coordinadorData ? coordinadorData.id_carrera : null;
            ...
            // Si no hay idCarrera, es reporte general (admin/rectora): solo la firma de la vicerrectora
            const soloVicerrectora = !idCarrera;
    ...
    async generarReportePDFPorCarrera(idPeriodo, idCarrera) {
        try {
            ...
            // Generar PDF con solo la firma de la vicerrectora
            const pdfBuffer = await this.crearPDF(asignacionesAgrupadas, periodo, autoridades, true);
    ...
    async crearPDF(asignacionesAgrupadas, periodo, autoridades, soloVicerrectora = false) {
        return new Promise((resolve, reject) => {
            try {
                ...
                // ✅ MODIFICADO: Definir posición X de la tabla para reutilizar en títulos
                const columnWidths = [25, 75, 75, 120, 50, 40, 40, 40];
    ...
    crearTabla(doc, asignaciones, periodo) {
        ...
        // Ajustar anchos de columna - más espacio para asignatura
        const columnWidths = [25, 75, 75, 120, 50, 40, 40, 40]; // Más espacio para asignatura, menos para otras
        const totalWidth = columnWidths.reduce((sum, width) => sum + width, 0);
        const startX = (doc.page.width - totalWidth) / 2; // Centrar tabla
        ...
        asignaciones.forEach((asig, index) => {
            ...
            rowData.forEach((data, colIndex) => {
                ...
                // Determinar alineación según la columna y longitud del texto
                let alignment = 'center';
                let textX = currentX + 3;

                // Para columnas con texto largo (evaluador, evaluado, asignatura), usar alineación izquierda
                if (colIndex === 1 || colIndex === 2 || colIndex === 3) {
                    const lineas = calcularLineasTexto(data, columnWidths[colIndex], 8);
                    if (lineas > 1) {
                        alignment = 'left';
                        textX = currentX + 4; // Más padding para texto izquierdo
                    }
                }

                // Calcular posición Y para centrar verticalmente
                const numLineas = calcularLineasTexto(data, columnWidths[colIndex], 8);
                const alturaTexto = numLineas * lineHeight;
                const textY = currentY + (rowHeight - alturaTexto) / 2 + 4; // +4 para ajuste visual
